...
class HCZ:
    ...
    def from_bounds(
        ...
        return cls.from_values(W_c=center, W_G=radius_expanded)
    ...
    def dual_upper(self, lmda: Float[Tensor, "j ..."]) -> Float[Tensor, "..."]:
        return (
            -norm(
                -t.sparse.mm(self.W_A, lmda) - self.W_G,  # i j, j ... -> i
                ord=1,
                dim=0,
            )
            - norm(
                -t.sparse.mm(self.W_Ap, lmda) - self.W_Gp,  # ip j, j ... -> ... ip
                ord=1,
                dim=0,
            )
            - self.W_C
    ...
    def intersect(
        ...
        **kwargs,
        ...
        if self.N == 0 or other.N == 0:
            return HCZ.empty()
        if r is not None:
            rg = t.sparse.mm(self.W_G, r)
            rgp = t.sparse.mm(self.W_Gp, r)
            rc = t.sparse.mm(self.W_C, r)
        else:
        ...
        if check_emptiness and result.is_empty(**kwargs):
            return HCZ.empty()
        ...
        return self.clone(
            ...
            * (
            ...
            W_Ap=self.cat(
                ...
                [
                    -1 / 2 * (self.W_B + self.W_Ap.sum(0)).unsqueeze(0),
                    1 / 2 * (other.W_B + other.W_Ap.sum(0)).unsqueeze(0),
